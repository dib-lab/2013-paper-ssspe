...
Some of the critera that wasn't being initially met but now are fulfilled:
\begin{inparaenum}[(1)] \item Comprehensive documentation (of the scripts).
  \item Documentation lists what version it applies to \item Automated build
  \item Documented build \item Dependency management \item Installer and 
  uninstaller \item Consistent copyright and license statements in all source
  files \item Both source and binary distributions \item Release checklist
  \item Coding standards conformance testing with enforcement \item Test
  coverage testing \item Continuous integration \item Email address for project
  \end{inparaenum}.
